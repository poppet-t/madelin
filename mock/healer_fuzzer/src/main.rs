...
struct Settings {
    ...
    /// Bridge bias JSON file generated from imported mock_seed.
    #[structopt(long)]
    bridge_bias: Option<PathBuf>,
    ...
    #[structopt(long)]
    debug: bool,
    /// Load config and print a deterministic summary without starting fuzzing.
    #[structopt(long)]
    dry_run: bool,
    /// Write the dry-run/debug summary as JSON.
    #[structopt(long)]
    debug_summary_json: Option<PathBuf>,
    /// Stop fuzzing cleanly after this many seconds.
    #[structopt(long)]
...
fn main() -> anyhow::Result<()> {
    ...
    let config = Config {
        ...
        bridge_bias: settings.bridge_bias,
        crash_whitelist: settings.crash_whitelist,
        job: settings.job,
        dry_run: settings.dry_run,
        debug_summary_json: settings.debug_summary_json,
