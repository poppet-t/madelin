...
use crate::{
    config::Config,
    crash::CrashManager,
    debug_summary::{build_debug_summary, render_debug_summary},
    feedback::Feedback,
...
use anyhow::Context;
use healer_core::{
    bridge_bias::{load_bridge_bias, set_bridge_bias},
...
use std::{
    ...
    fs::{create_dir_all, read_dir, read_to_string, rename, write, OpenOptions},
    io::BufWriter,
    os::raw::c_int,
    path::{Path, PathBuf},
    process::Command,
...
pub fn boot(mut config: Config) -> anyhow::Result<()> {
    ...
    let target_name = format!("{}/{}", config.os, config.arch);
    ...
    if let Some(path) = config.bridge_bias.as_ref() {
        let bias = load_bridge_bias(path).context("failed to load bridge bias")?;
        log::info!("loaded bridge bias for candidate {}", bias.candidate_id);
        set_bridge_bias(Some(bias));
    } else {
    ...
    let debug_summary = build_debug_summary(&config).context("failed to build debug summary")?;
    if let Some(path) = config.debug_summary_json.as_ref() {
        if let Some(parent) = path.parent() {
            if !parent.as_os_str().is_empty() {
                create_dir_all(parent).context("failed to create debug summary output directory")?;
            }
        }
        write(path, serde_json::to_vec_pretty(&debug_summary)?).context("failed to write debug summary json")?;
    }
    if config.dry_run {
        print!("{}", render_debug_summary(&debug_summary));
        return Ok(());
    }
    ...
    if let Some(max_seconds) = config.max_seconds {
        thread::spawn(move || {
            sleep(Duration::from_secs(max_seconds));
            log::info!("max-seconds reached ({}), stopping fuzzers", max_seconds);
            stop_req();
        });
    }

    let shared_state2 = SharedState::clone(&shared_state);
    // let dir_relation = config.output.clone().join("relation");
