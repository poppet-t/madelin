use crate::arch::host_target_arch;
use anyhow::Context;
...
pub struct Config {
    ...
    pub job: u64,
    pub dry_run: bool,
    pub debug_summary_json: Option<PathBuf>,
    ...
    pub bridge_bias: Option<PathBuf>,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            os: "linux".to_string(),
            arch: host_target_arch().to_string(),
            relations: None,
            input: None,
            crash_whitelist: None,
            job: 4,
            dry_run: false,
            debug_summary_json: None,
            max_seconds: None,
...
impl Config {
    fn validate_syz_dir(&self) -> anyhow::Result<()> {
        if !self.syz_dir.is_dir() {
            anyhow::bail!("bad syz-dir: {}", self.syz_dir.display());
        }

        resolve_executor_path(&self.syz_dir, &self.os, &self.arch)
            .map(|_| ())
            .map_err(|message| anyhow::anyhow!(message))
    }

    fn resolved_syz_executor(&self) -> anyhow::Result<PathBuf> {
        resolve_executor_path(&self.syz_dir, &self.os, &self.arch)
            .map_err(|message| anyhow::anyhow!(message))
    }

    pub fn check(&mut self) -> anyhow::Result<()> {
        let target_name = format!("{}/{}", self.os, self.arch);
        ...
        if let Some(i) = self.bridge_bias.as_ref() {
            if !i.is_file() {
                anyhow::bail!("bad bridge bias file: {}", i.display());
            }
        }

        self.repro_config.syz_dir = self.syz_dir.to_string_lossy().into_owned();
        self.report_config.syz_dir = self.syz_dir.to_string_lossy().into_owned();
    ...
    pub fn fixup(&mut self) {
        ...
        self.report_config.arch = self.arch.clone();
    ...
    pub fn syz_executor(&self) -> PathBuf {
        self.resolved_syz_executor()
            .unwrap_or_else(|_| fallback_syz_executor_path(&self.syz_dir, &self.os, &self.arch))
    }
}

fn fallback_syz_executor_path(syz_dir: &Path, os: &str, arch: &str) -> PathBuf {
    syz_dir
        .join("bin")
