...
impl ReproConfig {
    pub fn check(&mut self) -> Result<(), String> {
        ...
        resolve_tool_path(&syz_dir, "syz-repro").map(|_| ())
    }
}
...
pub fn repro(
    ...
    let syz_repro_path = resolve_tool_path(&config.syz_dir, "syz-repro")
        .map_err(|message| ReproError::Io(std::io::Error::new(std::io::ErrorKind::NotFound, message)))?;
    let syz_repro = Command::new(syz_repro_path)
        .arg("-config")
        .arg(&tmp_conf)
        .arg(&tmp_log)
        .output()?;
