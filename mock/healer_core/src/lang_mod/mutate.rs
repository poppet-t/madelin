...
use crate::{
    context::Context, mutation::seq::select_call_to, select::select_with_calls, syscall::SyscallId,
    RngType,
};

pub fn select_call_to_wrapper(
    ctx: &mut Context,
    rng: &mut RngType,
    idx: usize,
) -> (SyscallId, usize) {
...
pub fn select_call_to_lang(ctx: &mut Context, rng: &mut RngType, idx: usize) -> SyscallId {
    ...
    if idx != calls.len() {
        let mut back_calls: Vec<SyscallId> =
            calls[idx..].iter().rev().map(|c| c.sid() + 3).collect();
    ...
    let candidates: Vec<(SyscallId, f64)> =
        candidates.into_iter().collect::<Vec<(SyscallId, f64)>>();
...
pub fn top_k(pred: &Tensor, topk: i64) -> HashMap<usize, f64> {
    ...
    let candidates: HashMap<SyscallId, f64> = pred_indexes
        .into_iter()
