...
mod tests {
    ...
    fn test_accuracy() {
        ...
        let a: Tensor = Tensor::stack(
            &vec![
                Tensor::of_slice(&[0.3, 0.5, 0.2]),
                Tensor::of_slice(&[0.8, 0.2, 0.0]),
                Tensor::of_slice(&[0.1, 0.2, 0.7]),
            ],
            0,
        );
        let topk = vec![1, 2];
    ...
    fn test_train_model() {
        ...
        assert_eq!(
            model_file,
            String::from(
                "/home/model_manager/api/lang_model/checkpoints/syscall_model_jit_best.pt"
            )
        );
    }
