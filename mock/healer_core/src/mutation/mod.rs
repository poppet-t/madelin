...
pub fn mutate(
    ...
    type MutateOperation = fn(&mut Context, &CorpusWrapper, &mut RngType) -> (bool, usize);
    ...
    let mut ctx = Context::new(target, relation, model, scheduler);
    ...
    while tries < 128 && (!mutated || ctx.calls.is_empty() || rng.gen_ratio(1, 2)) {
        ...
        let (_mutated, _op) = OPERATIONS[idx](&mut ctx, corpus, rng);
        mutated = _mutated;
        op = _op;
...
pub fn restore_res_ctx(ctx: &mut Context, to: usize) {
