...
fn allows_generated_calls(call_idx: usize, generated_call_count: usize, prefix_len: usize) -> bool {
    generated_call_count == 0 || call_idx >= prefix_len
}

/// Select a call and mutate its args randomly.
pub fn mutate_call_args(
    ctx: &mut Context,
    ...
    if let Some(idx) = select_call(ctx, rng) {
        (do_mutate_call_args(ctx, rng, idx), 2)
    } else {
...
#[cfg(test)]
mod tests {
    use super::allows_generated_calls;

    #[test]
    fn prefix_calls_reject_helper_insertions() {
        assert!(!allows_generated_calls(1, 1, 3));
        assert!(allows_generated_calls(1, 0, 3));
        assert!(allows_generated_calls(3, 1, 3));
    }
}
