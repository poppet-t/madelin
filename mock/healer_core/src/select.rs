...
use crate::{
    bridge_bias::{syscall_bias_multiplier, weighted_syscalls},
    context::Context,
    gen::choose_weighted,
    lang_mod::mutate::top_k,
    syscall::SyscallId,
    HashMap, RngType,
};
use rand::prelude::*;
...
pub fn select_with_relation(ctx: &Context, rng: &mut RngType) -> Option<SyscallId> {
    ...
    for sid in calls.iter().map(|c| c.sid()) {
        for candidate in r.influence_of(sid).iter().copied() {
            ...
            *entry += syscall_bias_multiplier(ctx.target(), candidate);
        }
    }
...
pub fn select_random_syscall(ctx: &Context, rng: &mut RngType) -> Option<SyscallId> {
    if let Some(weighted) = weighted_syscalls(ctx.target()) {
        return weighted
            .choose_weighted(rng, |candidate| candidate.1)
            .ok()
            .map(|candidate| candidate.0);
