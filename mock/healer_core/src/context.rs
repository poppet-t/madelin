...
pub struct Context<'a, 'b> {
    ...
    /// Lang_model to mold syscalls.
    pub(crate) model: &'b ModelWrapper,
    /// Scheduler to balance exploration&exploit.
    pub(crate) scheduler: &'b Scheduler,
    /// Dummy mem allocator.
...
impl<'a, 'b> Context<'a, 'b> {
    ...
    pub fn new(
        target: &'a Target,
        relation: &'b RelationWrapper,
        model: &'b ModelWrapper,
    ...
    pub(crate) fn dummy() -> Self {
        ...
        let model: &'static ModelWrapper = unsafe { std::mem::transmute(dummy) };
        let scheduler: &'static Scheduler = unsafe { std::mem::transmute(dummy) };
        Self {
            target,
            relation,
            model,
